// secondary request meant to grab episodes if need but now is for grabbing the servers
...
let emptyKeyValue = [ new KeyValue( 'Referer', 'https://9animetv.to/' ), new KeyValue( 'X-Requested-With', 'XMLHttpRequest' ) ];
...
var type = '';
var episodes = [ ];
var json = document.body.querySelector('pre').innerText;
...
for ( var x = 0; x < animes.length; x++ ) {
    var anime = animes[ x ];
    var link = 'https://9animetv.to/ajax/episode/servers?episodeId=' + anime.dataset.id;
